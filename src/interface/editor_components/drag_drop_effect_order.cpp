...
void DragDropEffectOrder::setAllValues(vital::control_map& controls) {
  ...
  vital::utils::decodeFloatToOrder(effect_order_, order, vital::constants::kNumReorderableEffects);
...
void DragDropEffectOrder::moveEffect(int start_index, int end_index) {
  ...
  float order = vital::utils::encodeOrderToFloat(effect_order_, vital::constants::kNumReorderableEffects);
...
int DragDropEffectOrder::getEffectIndex(int index) const {
  int i = vital::utils::iclamp(index, 0, vital::constants::kNumReorderableEffects - 1);
...
int DragDropEffectOrder::getEffectIndexFromY(float y) const {
  float padding = size_ratio_ * kEffectPadding;
  int index = vital::constants::kNumReorderableEffects * (y + padding / 2.0f) / (getHeight() + padding);
  return vital::utils::iclamp(index, 0, vital::constants::kNumReorderableEffects - 1);
}

int DragDropEffectOrder::getEffectY(int index) const {
  int padding = size_ratio_ * kEffectPadding;
  return std::round((1.0f * index * (getHeight() + padding)) / vital::constants::kNumReorderableEffects);
}
