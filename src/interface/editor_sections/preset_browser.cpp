...
void PresetList::reloadPresets() {
  ...
  if (current_folder_.exists() && current_folder_.isDirectory())
    current_folder_.findChildFiles(presets_, File::findFiles, true, vital::kPresetExtensionsList);
  else
...
void PresetBrowser::jumpToPreset(int indices) {
  ...
  if (parent.exists()) {
    Array<File> presets;
    parent.findChildFiles(presets, File::findFiles, false, vital::kPresetExtensionsList);
