...
namespace {
  ...
  const SubMenu kDestinationSubMenuPrefixes[] = {
    ...
    { "delay_", "Delay" },
    { "dimension_expander_", "Dimension Expander" },
    { "distortion_", "Distortion" },
    { "phase_shift_", "Phase Shift" },
    { "frequency_shifter_", "Frequency Shifter" },
    { "phaser_", "Phaser" },
    { "flanger_", "Flanger" },
    { "limiter_", "Limiter" },
    { "reverb_", "Reverb" },
...
void ModulationMatrix::prevClicked() {
  File lfo_file = LoadSave::getShiftedFile(LoadSave::kLfoFolderName, vital::kLfoExtensionsList,
                                           "", getCurrentFile(), -1);
  if (lfo_file.exists())
    loadFile(lfo_file);
...
void ModulationMatrix::textMouseDown(const MouseEvent& e) {
  ...
  showPopupBrowser(this, bounds, LoadSave::getLfoDirectories(), vital::kLfoExtensionsList,
...
void ModulationMatrix::importLfo() {
  FileChooser import_box("Import LFO", LoadSave::getUserLfoDirectory(), vital::kLfoExtensionsList);
