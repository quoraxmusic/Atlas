...
namespace vital {
  ...
  FlangerModule::FlangerModule(const Output* beats_per_second, std::string prefix) :
      SynthModule(0, kNumOutputs), beats_per_second_(beats_per_second), prefix_(prefix),
  ...
  void FlangerModule::init() {
    ...
    Output* free_frequency = createMonoModControl(prefix_ + "flanger_frequency");
    frequency_ = createTempoSyncSwitch(prefix_ + "flanger", free_frequency->owner, beats_per_second_, false);
    center_ = createMonoModControl(prefix_ + "flanger_center");
    Output* feedback = createMonoModControl(prefix_ + "flanger_feedback");
    Output* wet = createMonoModControl(prefix_ + "flanger_dry_wet");
    mod_depth_ = createMonoModControl(prefix_ + "flanger_mod_depth");

    phase_offset_ = createMonoModControl(prefix_ + "flanger_phase_offset");
