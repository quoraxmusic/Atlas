...
namespace vital {
  ...
  DelayModule::DelayModule(const Output* beats_per_second, std::string prefix) :
      SynthModule(0, 1), beats_per_second_(beats_per_second), prefix_(prefix) {
  ...
  void DelayModule::init() {
    ...
    Output* free_frequency = createMonoModControl(prefix_ + "delay_frequency");
    Output* frequency = createTempoSyncSwitch(prefix_ + "delay", free_frequency->owner, beats_per_second_, false);
    Output* free_frequency_aux = createMonoModControl(prefix_ + "delay_aux_frequency");
    Output* frequency_aux = createTempoSyncSwitch(prefix_ + "delay_aux", free_frequency_aux->owner, beats_per_second_, false);
    Output* feedback = createMonoModControl(prefix_ + "delay_feedback");
    Output* wet = createMonoModControl(prefix_ + "delay_dry_wet");

    Output* filter_cutoff = createMonoModControl(prefix_ + "delay_filter_cutoff");
    Output* filter_spread = createMonoModControl(prefix_ + "delay_filter_spread");

    Value* style = createBaseControl(prefix_ + "delay_style");
