  ...
  public:
    ...
    void initialise(const String& command_line) override {
      ...
      if (command.contains(" --version ") || command.contains(" -v ")) {
        ...
        std::cout << "  --tabletowav                        Converts an Atlas wavetable to wav file." << newLine;
        ...
        for (const String& arg : args) {
          ...
          if (arg != "" && arg[0] != '-' && !last_arg_was_option && file.exists()) {
            ...
            if (extension == String(".") + String(vital::kWavetableExtension) ||
                extension == String(".") + String(vital::kLegacyWavetableExtension)) {
